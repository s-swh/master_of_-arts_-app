...
public class ArticlePreanter extends BasePreantert implements ArticleListContreater.IPreanter {
    ...
    @Override
    public void OnCommonSuccess(String token,int cat_id, int p, int per) {
        articleModel.OnCommonSuccess(token,cat_id, p, per, new ArticleListContreater.IModel.OnCommonCoallack() {
            @Override
