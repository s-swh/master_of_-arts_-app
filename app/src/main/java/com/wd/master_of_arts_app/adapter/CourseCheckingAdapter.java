...
public class CourseCheckingAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        ((CourseCheckingViewHorder)holder).title.setText(date+"\t\t"+start_time+"—"+end_time);
        ((CourseCheckingViewHorder) holder).my_te.setText(unit_name);

        int sort_num = dataBean.getSort_num();
        ((CourseCheckingViewHorder) holder).sort_num.setText("第"+sort_num+"课时");
        ((CourseCheckingViewHorder) holder).tv_name.setText(teacher_name);
        Glide.with(context).load(avatar).apply(RequestOptions.bitmapTransform(new CircleCrop())).into(((CourseCheckingViewHorder)holder).iv);
