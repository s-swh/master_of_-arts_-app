...
public interface Api {
    ...
    @FormUrlEncoded
    Observable<SMSLogin> getSmsLogin(@Field("phone") String phone, @Field("code") String code,@Field("registration_id")String registration_id);

    // 账号密码登录 http://test.54artist.com/wap/UserAccount/login
    @POST("wap/UserAccount/login")
    @FormUrlEncoded
    Observable<AccountLogin> getAccount(@Field("account") String account, @Field("pwd") String pwd,@Field("registration_id")String registration_id);
    ...
    @POST("wap/Qn/upload")
    @Multipart
    Observable<QiNiuYun> asr(@Part("dir")RequestBody dir,@Part MultipartBody.Part file);
    ...
    @FormUrlEncoded
    Observable<CommBean> getCommBean(@Field("token")String token,@Field("cat_id") int cat_id, @Field("p") int p, @Field("per") int per);
    ...
    //轮播图详情  http://test.54artist.com/wap/Banner/getMsg
    @POST("wap/Banner/getMsg")
    @FormUrlEncoded
    Observable<BannerPage>getBanner(@Field("id")int id);
    // 热更新 /wap/version/getMsg
